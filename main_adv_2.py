...
def parse_option():
    ...
    parser.add_argument('--adv_upweight', type=int, default=1,
    ...
    opt.model_name = 'GraphCE_{}_model_{}_temp_{}_alpha_{}_beta_{}'.\
        format(opt.dataset, opt.model, opt.temp, opt.weight, opt.weight1)
...
def contrastive_loss(clean_features, adv_features, labels, temp, adv_connect, upweight):
    ...
    criterion = GraphLoss(temp, base_temperature = temp)
...
def contrastive_train(train_loader, model, optimizer, epoch, opt):
    ...
    for idx, (images, labels) in enumerate(train_loader):
        ...
        # clean loss
        loss_2 = F.cross_entropy(logits_adv, labels)
...
def main(opt):
    ...
    if (opt.dataset == 'cifar10'):
        lr_scheduler = torch.optim.lr_scheduler.MultiStepLR(optimizer, milestones=[70, 100], gamma=0.1)
